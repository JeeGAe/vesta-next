                    ...
                    {weekCountArray.map((week, id) => {
                        ...
                        return (
                            <tr key={id}>{innerDate.map((d, index) => {
                                return (
                                    <td key={index} className={`min-h-date min-w-date h-date text-center cursor-pointer ${props.date === d && 'bg-primary-color'}`} onClick={() => props.selectDate(d)}>{d === -1? '' : d}</td> // 1주일 씩 자른 배열을 td 태그에 하나씩 일 수를 채워 넣어줍니다.
                                )
                            })}</tr>
                        )
                    })}
